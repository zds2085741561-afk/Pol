...
DECODER_VERSION = "v7-s3-kf-strict-tf-sid-cpr"
...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--checkpoint",
        default="",
        help="trained S3-KF StuRec checkpoint; safe defaults are provided for NYC and NYC_STEPS",
    )
...
def resolve_checkpoint(project_root, dataset_name, explicit_path, base_model="teacher"):
    ...
    if base_model == "teacher":
        ...
        known["NYC"] = os.path.join(
            project_root, "saved", "StuRec-Jul-13-2026_12-46-28.pth"
        )
        known["NYC_STEPS"] = os.path.join(
            project_root, "saved", "StuRec-Jul-13-2026_17-10-25.pth"
        )
...
def student_kwargs_from_checkpoint(base_kwargs, checkpoint, state_dict, args):
    ...
    # Pure TF-KD students do not use the graph in their inference scorer, but
    # research-point-2 still needs the frozen train-derived graph for GTF and
    # TF-SID path constraints. Keeping this flag only retains graph buffers;
    # it does not alter forward scoring or add a training loss during eval.
    kwargs["use_tfkd"] = as_bool(
        first_checkpoint_value(checkpoint_config, ["use_tfkd"], False),
        default=has_graph_buffer,
    ) or has_graph_buffer
...
def load_student_model(project_root, dataset_name, checkpoint_path, args):
    ...
    print(
        "[Model] Loaded frozen S3-KF Student checkpoint. "
...
def main():
    ...
    print(
        "[Experiment] C0-C4 share the same frozen S3-KF Student; "
